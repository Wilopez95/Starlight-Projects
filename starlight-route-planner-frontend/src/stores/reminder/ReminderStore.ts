...
export class ReminderStore extends BaseStore<Reminder> {
  ...
  async createReminderSchedule(reminderConfig: IConfigurableReminderSchedule) {
    ...
    try {
      ...
      NotificationHelper.success('default', `Reminder schedule #${reminderSchedule.id}`);
      ...
      NotificationHelper.error('default', typedError.response.code as ActionCode, 'Reminder');
    } finally {
      this.loading = false;
    }
  }
  ...
  async deleteReminderSchedule(id: number) {
    ...
    try {
      ...
      NotificationHelper.success('default', `Reminder schedule #${id}`);
  ...
  async clearAllUserReminders() {
    ...
    try {
      ...
      NotificationHelper.success('default', `Reminders`);
    } catch (error: unknown) {
      const typedError = error as ApiError;
      NotificationHelper.error('default', typedError.response.code as ActionCode);
    } finally {
