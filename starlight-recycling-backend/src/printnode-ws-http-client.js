...
const PrintNode = (function () {
  ...
  publify.topicExplodeHierarchy = function (topic) {
    ...
    output = [publify.escapeTopicFragment(topicComponents[0])];
  ...
  function getWSSendFn(soc, defaultAck, ackTimeout, errCallback, logSend) {
    ...
    function send(message, payload, ack) {
      var ackKey,
        ackSource = undefined === ack ? defaultAck : ack;
      var when = Date.now();
      msgCount++;
      ...
      if (ackSource) {
        ...
        ackHistory[ackKey] = [
          setTimeout(function () {
            ...
            errCallback(new PN_Error('Server', 'No ack recieved'), {
              timeout: true,
      ...
      try {
        ...
        errCallback(new PN_Error('Socket', "Send failed. It is likely the socket isn't open"), {
          timeout: false,
          message: message,
          payload: payload,
        });
    ...
    send.ack = function (ackKey) {
      ...
      if (undefined === ack) {
        errCallback(
          new PN_Error(
            'Server',
            "Unexpected ack recieved; either this message didn't request a ack or we've already recieved it.",
          ),
          { timeout: false },
        );
        return;
  ...
  ScalesMeasurement.prototype.getLatency = function () {
    var client = new Date(this.clientReportedCreateTimestamp),
      now = new Date();
  ...
  ComputerConnections.prototype.add = function (cc) {
    if (!(cc instanceof Connection)) {
      throw new PN_Error(
        'Runtime',
        'You can only add a Connection object to a ComputerConnections array',
      );
    }
    ...
    if (null == this.computerId || (this.computerId === 0 && computerId > 0)) {
      ...
      throw new PN_Error(
        'Runtime',
        'Attempting to add Connection object to a ComputerConnections array with different computerId',
      );
    }
    this.push(cc);
  };
  ...
  ComputerConnections.factory = function (data) {
    ...
    if (!_isArray(connections)) {
      throw new PN_Error(
        'Server',
        "Server payload for ComputerConnections, 'connections' property isn't a array",
      );
    }
    if (!_isInt(accountId)) {
      throw new PN_Error(
        'Server',
        "Server payload for ComputerConnections, 'accountId' should be a int",
      );
    }
    if (!_isInt(computerId)) {
      throw new PN_Error(
        'Server',
        "Server payload for ComputerConnections, 'computerId' should be a int",
      );
    }
  ...
  function generateScalesUrlFromOptions(options, allowNoOptions) {
    ...
    if (0 === keys.length && allowNoOptions) {
      ...
      path = [
        'computer',
        encodedOptions.computerId,
        'scale',
        encodedOptions.deviceName,
        encodedOptions.deviceNum,
      ];
      ...
      throw new PN_Error(
        ...
        'Options key combination (' +
          keys.join(', ') +
          ') for getting scales data; please refer to documentation.',
      );
    }
    return path.join('/');
  }
  ...
  function generateComputerConnectionsUrlFromOptions(options, allowNoOptions) {
    ...
    if (0 === keys.length && allowNoOptions) {
      ...
      throw new PN_Error(
        ...
        'Options key combination (' +
          keys.join(', ') +
          ') for getting computer connections is invalid; please refer to documentation.',
      );
    }
    return path.join('/');
  }
  ...
  function PN_WebSocket(options, authCallback, errorCallback) {
    ...
    if (!options || !options.apiKey) {
      throw new PN_Error(
        'BadArguments',
        "Options argument expected to be a object with property 'apiKey'",
      );
    }
    ...
    function authenticateTimeout() {
      ...
      // wrap this up
      failHandler(new PN_Error('Server', 'Server failed to authenticate in time'), payload);
    }

    // internal protocol level messages
    function onopen(evt) {
      isConnected = true;
      authenticateTimeout.timeout = setTimeout(authenticateTimeout, options.authTimeout);
    ...
    function onerror(evt) {
      ...
      failHandler(new PN_Error('Socket'), { socket: true, socketError: evt });
    }
    ...
    function onmessage(evt) {
      ...
      switch (message.typ) {
        ...
        case 'protocol':
          ...
          break;
        case 'publish':
          var topics = message.message,
            subId = message.payload[0],
            payload;
    ...
    var startWs = function startWs(webSocketUrl) {
      ...
      this.getScales = function (options, callback, ctx) {
        ...
        return makeServerSubscription('/' + path + '/', callback, ctx, ScalesMeasurement.factory, [
          'scales',
        ]);
      };
    ...
    if (options.server) {
      var webSocketUrl = ['wss://', options.server, '/ws/', options.version].join('');
    ...
    function fallback(reason) {
      ...
      if (console && console.log) {
        console.log(
          'Using api.printnode.com for websocket because; ' +
            reason +
            '. This is expected prior to 2020-12-27.',
        );
      }
    ...
    var centralUrl =
      'https://' + options.centralOrigin + '/v3/proxy?key=' + encodeURIComponent(apiKey);
            ...
            ajax(
              {
                ...
                success: function (responseBody, response) {
                  ...
                  var webSocketUrl =
                    'wss://' + responseBody.httpPublicHost + '/ws/' + options.version;
                  startWs(webSocketUrl);
                },
  ...
  AjaxResponse.prototype.parseResponseBody = function () {
    var contentTypeHeader = this.xhr.getResponseHeader('Content-Type'),
      response;
    ...
    try {
      ...
      this.bodyParsingError =
        'Server has not responded valid JSON, it returned; ' + this.xhr.responseText;
    }
  };
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState == 4) {
        ...
        if (response.isOk()) {
          ...
          promise(true, [response.response, response]);
        } else {
          if (options.error) {
            try {
          ...
          promise(false, [response.response, response]);
        }
        if (options.complete) {
          try {
...
// @ts-check
/** @type {{ WebSocket: PrintNodeClient.WebSocketConstructor }} */
module.exports = { WebSocket: PrintNode.WebSocket };
