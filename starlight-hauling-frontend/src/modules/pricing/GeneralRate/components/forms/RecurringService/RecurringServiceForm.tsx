...
import { type IRecurringServiceForm, type IRecurringServiceGeneralRateFormikData } from './types';
...
const RecurringServiceForm: React.FC<IRecurringServiceForm> = ({
  ...
  const { values, errors, handleChange, handleBlur } =
    useFormikContext<IRecurringServiceGeneralRateFormikData>();
  const { generalRateStoreNew, billableServiceStore, equipmentItemStore, materialStore } =
    useStores();
    ...
    (async () => {
      if (
        ...
        const fetchedFrequencies = await Promise.all(
          values.recurringService?.map(service => {
            if (!service.frequencyId) {
  ...
  const getBillableServiceWithIncludedServicesLabel = useCallback(
    (service: IRecurringServiceGeneralRate): [IExtendedBillableService, string | undefined] => {
                            ...
                            <Table className={styles.table}>
                              <tbody>
                                {service.frequencies?.map((frequency, frIndex) => {
                                  const inputPath = `recurringService[${index}].frequencies[${frIndex}].price`;
