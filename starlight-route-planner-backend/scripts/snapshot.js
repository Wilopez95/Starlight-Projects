...
import generateSnapshot from '../db/generateSnapshot.js';
...
yargs(hideBin(process.argv))
  ...
  .command('generate', 'Update DB snapshots', async () => {
    const { knex } = dbManager;

    await generateSnapshot(knex);
  ...
  .command(
    ...
    async args => {
      const allTables = await getAllTablesAndColumns(dbManager.knex);
      ...
      Object.entries(tables).forEach(([tableName, { attributes, constraints }]) => {
        if (tableName.endsWith('historical')) {
          return;
        }
