...
export const getValues = (balance: number, payout: Payout | null): NewPayout => {
  ...
  const newValues = notNullObject<NewPayout>(payout, defaultValue);

  if (newValues.paymentType === 'creditCard' && payout.creditCard) {
    newValues.creditCardId = +payout.creditCard.id;
  }
