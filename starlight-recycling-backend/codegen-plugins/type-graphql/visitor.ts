...
> extends TsVisitor<TRawConfig, TParsedConfig> {
  ...
  ObjectTypeDefinition(node: ObjectTypeDefinitionNode, key: number | string, parent: any): string {
    let isGraphQLType = GRAPHQL_TYPES.includes((node.name as unknown) as string);

    // in billing, guys are breaking graphql by using entity with name Subscription
  ...
  FieldDefinition(
    ...
    const decorator =
      '\n' +
      indent(
        `@TypeGraphQL.${fieldDecorator}(() => ${fieldType}${formatDecoratorOptions(
          decoratorOptions,
          false,
        )})`,
      ) +
    ...
    return (
      decorator +
      indent(`${this.config.immutableTypes ? 'readonly ' : ''}${node.name}!: ${typeString};`)
    );
  }
  ...
  InputValueDefinition(
    ...
    return (
      decorator +
      indent(`${this.config.immutableTypes ? 'readonly ' : ''}${nameString}!: ${typeString};`)
    );
  }
  ...
  UnionTypeDefinition(node: UnionTypeDefinitionNode, key, parent): string {
    ...
    const possibleTypes = originalNode.types.map((t) =>
      this.scalars[t.name.value] ? this._getScalar(t.name.value) : this.convertName(t),
    );
