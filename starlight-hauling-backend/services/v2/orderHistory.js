...
export const getHistoricalRecords = async (ctx, orderId) => {
  ...
  const orders = await knex(OrderRepo.getHistoricalTableName())
  ...
  const [lineItems, thresholdItems] = await Promise.all([
    knex(LineItemRepo.getHistoricalTableName())
      .withSchema(schemaName)
      .where({ orderId })
      .select('*')
      .orderBy('id', 'desc'),
    knex(ThresholdItemRepo.getHistoricalTableName())
      .withSchema(schemaName)
      .where({ orderId })
      .select('*')
      .orderBy('id', 'desc'),
  ...
  const refactoredOrders = replaceLegacyWithRefactoredFieldsDeep(orders);
  const refactoredLineItems = replaceLegacyWithRefactoredFieldsDeep(lineItems);
  const refactoredThresholdItems = replaceLegacyWithRefactoredFieldsDeep(thresholdItems);

  // compare prev & new values of all order-related entities
  const comparedOrders = getComparedSingleEntityRecords(
    refactoredOrders,
    HISTORICAL_ORDER_ENTITY.order,
  );
  const comparedLineItems = lineItems?.length
    ? getComparedMultiEntitiesRecords(refactoredLineItems, HISTORICAL_ORDER_ENTITY.lineItem)
    : [];
  const comparedThresholdItems = thresholdItems?.length
    ? getComparedMultiEntitiesRecords(
        refactoredThresholdItems,
        HISTORICAL_ORDER_ENTITY.thresholdItem,
      )
    : [];
