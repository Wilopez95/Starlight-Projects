...
import { SUBSCTIPTION_HISTORY_ENTITIES } from '../../../consts/subscriptionHistoryEntities.js';
import { SUBSCTIPTION_HISTORY_ENTITY_ACTIONS } from '../../../consts/subscriptionHistoryEntityActions.js';
import {
  SUBSCRIPTION_HISTORY_DEFAULT_SORTING,
  SUBSCRIPTION_HISTORY_SORT_KEYS,
...
export const editSubscriptionData = Joi.object()
  .keys({
    ...
    unlockOverrides: Joi.boolean(),
    promoId: id.allow(null),
    customRatesGroupId: id.allow(null, 0),
    serviceItems: Joi.array().items(
      Joi.object().keys({
...
export const getSubscriptionHistorySchema = Joi.object().keys({
  skip: Joi.number().integer().positive().allow(0),
  limit: Joi.number().integer().positive(),

  sortBy: Joi.string()
    .valid(...SUBSCRIPTION_HISTORY_SORT_KEYS)
    .optional()
    .default(SUBSCRIPTION_HISTORY_DEFAULT_SORTING),
  sortOrder: Joi.string()
    .valid(...SORT_ORDERS)
    .optional()
    .default(SORT_ORDER.desc),

  action: Joi.string().valid(...SUBSCTIPTION_HISTORY_ACTIONS),
  attribute: Joi.string(),
  entity: Joi.string().valid(...SUBSCTIPTION_HISTORY_ENTITIES),
  entityAction: Joi.string().valid(...SUBSCTIPTION_HISTORY_ENTITY_ACTIONS),
  createdAtFrom: Joi.date(),
  createdAtTo: Joi.date(),
  effectiveDateFrom: Joi.date(),
  effectiveDateTo: Joi.date(),
  madeByIds: Joi.array().items(Joi.string()).single(),
});

export const getBySearchQuery = Joi.object().keys({
