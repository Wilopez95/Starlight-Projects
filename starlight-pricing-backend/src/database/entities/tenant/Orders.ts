...
@Entity()
@Check(
  `"cancellation_reason_type" IN ('customerCanceled', 'duplicateOrder', 'schedulingError', 'internalError', 'other')`,
)
@Check(`"notify_day_before" IN ('byText', 'byEmail')`)
@Check(`"payment_method" IN ('onAccount', 'creditCard', 'cash', 'check', 'mixed')`)
@Check(`"status" IN ('inProgress', 'completed', 'approved', 'finalized', 'canceled', 'invoiced')`)
export class Orders {
  @PrimaryGeneratedColumn()
  id: number;
  ...
  @ManyToOne(_ => OrderRequests, request => request.id)
  @JoinColumn({ name: 'order_request_id' })
  orderRequestFK: OrderRequests;
  ...
  @Column({ type: 'text', nullable: false, default: 'inProgress' })
  status: string;
  ...
  @Column({ type: 'int4', nullable: false, name: 'customer_job_site_id' })
  ...
  //priceFK: Prices; // refactor starts here
