...
import { JsonConversions } from '@root/auth/types/JsonConversions';
...
export class HttpClient {
  ...
  private graphqlClient?: 'trashapi';

  constructor(apiUrl: string, graphqlClient?: 'trashapi') {
    this.apiUrl = apiUrl;
  ...
  private executeWithMiddleware<
    TArg extends RequestOptions<unknown> | GraphqlRequestOptions,
  >(func: (arg: TArg) => Promise<unknown>, arg: TArg) {
  ...
  private executeGraphqlRequest<R>(
    ...
    if (!this.graphqlClient) {
      throw new Error(
        'You can not use GraphQL with services that do not support it',
      );
    }
    // @ts-expect-error
