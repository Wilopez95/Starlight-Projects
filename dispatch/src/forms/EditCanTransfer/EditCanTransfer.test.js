...
import EditCanTransfer from './';

describe('<EditCanTransfer />', () => {
  const initialState = {
    trucks: [
      {
        createdBy: 'ids',
  ...
  const testInstance = (props, state) => {
    ...
    const provider = mount(
      <Provider store={store}>
        <EditCanTransfer {...props} dispatch={store.dispatch} />
      </Provider>,
    );

    const wrapper = provider.find('EditCanTransfer');
  ...
  it('should be successfully mounted', () => {
    ...
    expect(wrapper.find('.form--editCan-transfer')).toHaveLength(1);
  });
