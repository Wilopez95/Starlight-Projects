...
class ColumnBuilder {
  ...
  build(tableChain) {
    ...
    if (!this.hasSpecifiedType() && this.tasks.length > 0) {
      throw new MigrationError(
        `You must specify a column type first! At column ${this.tableBuilder.tableName}.${this.columnName}`,
      );
    }
  ...
  identity({ constraintName, kind = IDENTITY_KIND.byDefault } = {}) {
    ...
    this.tableBuilder.primary([this.columnName], constraintName);
  ...
  timenotz() {
    return this.type('time without time zone');
  }
  ...
  timestampnotz() {
    return this.type('timestamp without time zone');
  }
  ...
  geography() {
    return this.type('public.geography');
  }
