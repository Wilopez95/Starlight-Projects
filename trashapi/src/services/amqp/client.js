...
import { AMQP_HOST, AMQP_PORT, AMQP_USER, AMQP_PASSWORD } from '../../config.js';

const AMQP_URL = `amqp://${AMQP_USER}:${AMQP_PASSWORD}@${AMQP_HOST}:${AMQP_PORT}`;
  ...
  async connect(prefetchLimit) {
    if (!this.connection) {
      try {
        ...
        logger.error(`Failed to connect via amqp for URL, host: ${AMQP_HOST}`);
        throw error;
      }
