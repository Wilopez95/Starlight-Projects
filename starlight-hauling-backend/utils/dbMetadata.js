...
export const getAllTablesAndColumns = async knex => {
  ...
  await knex.transaction(async trx => {
    ...
    [columns, constraints, indexes] = await Promise.all([
      trx('pg_attribute as att')
        .select(
          'rel.relname',
          'nsp.nspname',
          'att.attname as name',
          ...
          'typ.typname as type',
          trx.raw('pg_get_expr(??, ??) as ??', ['ad.adbin', 'rel.oid', 'default_value']),
        )
        ...
        .leftJoin('pg_attrdef as ad', builder =>
          builder
            .on(trx.raw('?? is true', ['att.atthasdef']))
        ...
        .orderBy('att.attnum'),
      trx('pg_constraint as con')
        .select(
          'nspname',
          'contype',
          'conname as name',
          'relname',
          trx.raw('pg_get_constraintdef(??, ?) as ??', ['con.oid', 'true', 'definition']),
        )
      ...
      trx('pg_index as ind')
        .select(
          ...
          'rel.relname',
          trx.raw('pg_get_indexdef(??) as ??', ['ind.indexrelid', 'definition']),
        )
...
export const getAllTablesAndColumnsBySchema = async (knex, targetSchemaName) => {
  let columns;
  let constraints;
  let indexes;
  ...
  await knex.transaction(async trx => {
    [columns, constraints, indexes] = await Promise.all([
      trx('pg_attribute as att')
        ...
        .where('nsp.nspname', targetSchemaName)
        .andWhere('att.attnum', '>', 0)
        .andWhere('rel.relkind', 'r')
        .orderBy('att.attnum'),
      trx('pg_constraint as con')
      ...
      trx('pg_index as ind')
        ...
        .innerJoin('pg_class as indrel', 'indrel.oid', 'ind.indexrelid')
        .innerJoin('pg_namespace as nsp', 'nsp.oid', 'rel.relnamespace')
        .where('nspname', targetSchemaName)
        .andWhere('rel.relkind', 'r')
        .andWhere(
          'ind.indexrelid',
          'not in',
