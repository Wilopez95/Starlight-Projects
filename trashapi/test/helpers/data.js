...
import R, { compose as o, identity as I } from 'ramda';
...
export const clear = async () => {
  const results = await my(
    ...
    WHERE table_schema='${config.get('db.database')}'
  `),
  );
...
export const point = bbox => {
  ...
  } = turf.random('points', 1, { bbox });
  return { lat, lon };
};

export const date = (from, to) => `${format(from, 'T')}..${format(to, 'T')}`;

export const isOdd = R.modulo(R.__, 2);

export const { 0: evens, 1: odds } = R.countBy(isOdd, R.times(I, all));

const planTests = type => R.times(R.concat([type]), all);

export const toCreate = o(R.chain(I), R.map(planTests), R.keys);
...
export const dates = o(...R.map(toUTC, ['modifiedDate', 'createdDate']));
