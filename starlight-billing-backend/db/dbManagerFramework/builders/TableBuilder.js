...
class TableBuilder {
  ...
  unique(columnNames, { constraint = true, indexType = 'btree' } = {}) {
    ...
    if (constraint) {
      ...
      this.index(columns, { indexType, unique: true });
    }
  }
  ...
  addHistoricalTable() {
    this.schemaTasks.push({
      ...
      perform: async () => {
        ...
        historicalTableBuilder.column('user_id').text().defaultTo('system').notNullable();
