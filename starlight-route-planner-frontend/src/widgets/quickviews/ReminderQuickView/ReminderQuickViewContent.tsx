...
import { Divider, TableInfiniteScroll, TableScrollContainer } from '@root/common/TableTools';
...
const ReminderQuickViewContent: React.FC = () => {
  ...
  const handleClearAll = async () => {
    if (hasReminders) {
      await reminderStore.clearAllUserReminders();
    }
  };

  const loadMore = useCallback(async () => {
    await reminderStore.getAllUserReminders();
  }, [reminderStore]);
  ...
  return (
    <div className={styles.sidebar}>
      ...
      <TableScrollContainer className={styles.tableContainer}>
        {hasReminders ? (
          <>
