...
/* eslint-disable  camelcase, no-unused-expressions, no-multi-assign */
...
class Toast extends Component {
  ...
  render() {
    ...
    const toastProps = {
      className: cx(
        'sl-notif__toast',
        `sl-notif__toast--${type}`,
        {
          'sl-notif__toast--rtl': rtl,
        },
        className,
      ),
    };
    ...
    return (
      ...
      >
        ...
        >
          <div
            {...(this.props.in && { role })}
            className={cx('sl-notif__toast-body', bodyClassName)}
          >
            {children}
          </div>
          {closeButton === false ? null : closeButton}
          {autoClose === false ? null : <ProgressBar
              key={`pb-${updateId}`}
              rtl={rtl}
              delay={autoClose}
