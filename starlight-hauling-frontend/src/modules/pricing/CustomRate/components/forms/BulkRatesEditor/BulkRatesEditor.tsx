...
import { calculateFinalPrice } from '../../../../GeneralRate/helpers';
import { type ILineItemCustomRateFormikData } from '../LineItem/types';
import { type IRecurringLineItemCustomRateFormikData } from '../RecurringLineItem/types';
import { type IRecurringServiceCustomRateFormikData } from '../RecurringService/types';
import { type IServiceCustomRateFormikData } from '../Service/types';
...
const BulkRatesEditor: React.FC<IBulkRatesEditor> = ({
  ...
  const { values, setFieldValue, setErrors, errors } = useFormikContext<
    | IServiceCustomRateFormikData
    | IRecurringServiceCustomRateFormikData
    | ILineItemCustomRateFormikData
    | IRecurringLineItemCustomRateFormikData
  >();
        ...
        currentRates.map(rateItem => ({
          ...
          finalPrice:
            rateItem.price && validator.isNumeric(value)
              ...
              : '',
          // billingCycles: (rateItem as IRecurringLineItemCustomRate).billingCycles?.map(
          //   (billingCycleObj) => {
          //     return {
          //       ...billingCycleObj,
          ...
          //   },
          // ),
          frequencies: (rateItem as IRecurringServiceCustomRate).frequencies?.map(frequency => {
            return {
              ...frequency,
  ...
  return (
    <Layouts.Margin top="1" bottom="1">
      <Layouts.Grid {...gridFormat}>
        ...
        <Layouts.Box />
        {prop === RatesEntityType.oneTimeLineItem || prop === RatesEntityType.recurringLineItem ? (
          <Layouts.Box />
        ) : null}
