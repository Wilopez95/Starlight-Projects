...
describe('Re-schedule 1 PT Delivery Order Without PO: Default Flow V2', () => {
  ...
  it(`should reschedule delivery order`, async () => {
    const { statusCode } = await client
      ...
      .send(ordersFixtures.reschedulePtOrderInputDefault);
    expect(statusCode).toEqual(httpStatus.OK);
  ...
  it(`should get newly updated delivery order and ensure that updated properties are correct`, async () => {
    ...
    expect(details.rescheduleComment).toEqual(ordersFixtures.reschedulePtOrderInputDefault.comment);
    expect(details).toHaveProperty('serviceDate');
    expect(details.serviceDate).toEqual(ordersFixtures.reschedulePtOrderInputDefault.serviceDate);
    expect(details).toHaveProperty('customer');
    expect(details.customer).toHaveProperty('originalId');
    expect(details.customer.originalId).toEqual(
      ordersFixtures.newPtDeliveryOrderInputDefault.customerId,
    ...
    expect(details.isRollOff).toEqual(false);
    expect(details).toHaveProperty('grandTotal');
    expect(details.grandTotal).toEqual(19 * 1_000_000); // only price of trip charge line itemand surcharge amount (if exists)
    expect(details).toHaveProperty('independentWorkOrder');
