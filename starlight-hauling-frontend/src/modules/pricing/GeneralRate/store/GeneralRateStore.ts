...
import { GeneralRateService } from '../api/generalRate';
import { IGeneralRatePayload, IGeneralRateRequestParams } from '../api/types';
import { sanitizeRates } from '../helpers';

const defaultValue: IGeneralRatePayload = {
...
export class GeneralRateStoreNew extends BaseStore<BaseEntity> {
  private readonly service: GeneralRateService;

  @observable rates: IGeneralRatePayload;

  constructor(global: GlobalStore) {
    super(global);
    this.service = new GeneralRateService();
    this.loading = false;
    this.rates = defaultValue;
  }

  @actionAsync
  async request(data: IGeneralRateRequestParams) {
    this.loading = true;
    try {
      ...
      this.rates = {
        ...defaultValue,
        [data.entityType]: requestResponse,
      };
    } catch (error: unknown) {
  ...
  @actionAsync
  async update(data: Partial<IGeneralRatePayload> & IBusinessContextIds) {
    try {
