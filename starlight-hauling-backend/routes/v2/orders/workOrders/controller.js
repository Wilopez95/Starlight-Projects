...
export const autoSyncWithDispatch = async ctx => {
  ...
  const order = await orderRepo.getBy({ condition: { woNumber } });
...
const getBlIdAndSchema = async ctx => {
  ...
  if (woNumber) {
    ...
    const order = await OrderRepo.getInstance(ctx.state, { schemaName }).getBy({
      condition: { woNumber },
      fields: ['id', 'workOrderId', 'businessLineId'],
    });
    if (!order) {
      throw orderNotFound();
    }
