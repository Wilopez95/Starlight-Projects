...
const DefaultWorkOrderData: React.FC<IWorkOrderDataProps> = ({
  ...
  const disposalOptions: ISelectOption[] = sortBy(disposalSiteStore.values, ['description']).map(
    disposal => ({
      label: disposal.description,
      value: disposal.id,
    }),
  );
...
export default observer(DefaultWorkOrderData);
