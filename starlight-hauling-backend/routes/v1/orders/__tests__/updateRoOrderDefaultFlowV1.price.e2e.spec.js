...
describe('Update 1 Ro Delivery Order With PO: Default Flow V1', () => {
  ...
  it(
    ...
    async () => {
      const { statusCode, body: item } = await client
        .post(`${URL_PREFIX}`)
        .send(ordersFixtures.newRoDeliveryOrderToUpdateInputDefault);
  ...
  it(`should get newly created delivery order and ensure that the main structure and params are correct`, async () => {
    ...
    expect(details.serviceDate).toEqual(
      ordersFixtures.newRoDeliveryOrderToUpdateInputDefault.orders[0].serviceDate,
    );
    expect(details).toHaveProperty('driverInstructions');
    expect(details.driverInstructions).toEqual(
      ordersFixtures.newRoDeliveryOrderToUpdateInputDefault.orders[0].driverInstructions,
    );
    ...
    expect(details.customer.originalId).toEqual(
      ordersFixtures.newRoDeliveryOrderToUpdateInputDefault.customerId,
    );
    ...
    expect(details.jobSite.originalId).toEqual(
      ordersFixtures.newRoDeliveryOrderToUpdateInputDefault.orders[0].jobSiteId,
    );
    ...
    expect(details.businessUnit.id).toEqual(
      ordersFixtures.newRoDeliveryOrderToUpdateInputDefault.businessUnitId,
    );
    ...
    expect(details.businessLine.id).toEqual(
      ordersFixtures.newRoDeliveryOrderToUpdateInputDefault.businessLineId,
    );
    ...
    expect(details.serviceArea.originalId).toEqual(
      ordersFixtures.newRoDeliveryOrderToUpdateInputDefault.orders[0].serviceAreaId,
    );
    ...
    expect(details.billableService.originalId).toEqual(
      ordersFixtures.newRoDeliveryOrderToUpdateInputDefault.orders[0].billableServiceId,
    );
    ...
    expect(details.material.originalId).toEqual(
      ordersFixtures.newRoDeliveryOrderToUpdateInputDefault.orders[0].materialId,
    );
    ...
    expect(details.equipmentItem.originalId).toEqual(
      ordersFixtures.newRoDeliveryOrderToUpdateInputDefault.orders[0].equipmentItemId,
    );
    ...
    expect(details.grandTotal).toEqual(
      ordersFixtures.newRoDeliveryOrderToUpdateInputDefault.orders[0].grandTotal,
    );
