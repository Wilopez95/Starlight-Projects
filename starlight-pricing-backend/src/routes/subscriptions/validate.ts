...
import {
  ...
  BILLING_TYPES_VALUES,
  EQUIPMENT_TYPES,
  PAYMENT_METHODS,
} from '../../consts/subscriptions';
import { SUBSCRIPTION_SORT_KEYS } from '../../consts/subscriptionAttributes';
import { SORT_ORDERS } from '../../consts/sortOrders';
import { SUBSCRIPTION_STATUSES } from '../../consts/subscriptionStatuses';
...
const requiredIfIsNull = (field: string) =>
  Joi.when(field, {
    is: id.required(),
    then: id.allow(null),
  }).concat(
...
export const createSubscriptionData = Joi.object()
  .keys({
    ...
    serviceItems: Joi.array().items(
      Joi.object()
        .keys({
          billableServiceId: Joi.number().required(),
          ...
          lineItems: Joi.array().items(
            Joi.object()
              .keys({
                billableLineItemId: Joi.number().required(),
        ...
        lineItems: Joi.array().items(
          Joi.object()
            .keys({
              ...
              customRatesGroupRecurringLineItemBillingCycleId: Joi.number().allow(null),
              price: positiveFloat.required().allow(0),
              unlockOverrides: Joi.boolean().default(false),
              quantity: Joi.number().positive().required(),
            })
...
export const simpleSearchParams = commonFilters.keys({
  businessUnitId: id.required(),
