...
import { EVENT_TYPES } from '../../../consts/historicalEventType.js';
...
class TableBuilder {
  ...
  primary(columns, constraintName = undefined) {
    this.tableTasks.push({
      type: TABLE_TASK_TYPE.addConstraint,
      kind: CONSTRAINT_KIND.primary,
      perform: t => {
        t.primary(columns, constraintName);
      },
    });
  }

  unique(columnNames, { constraint = true, indexType = 'btree', indexName = undefined } = {}) {
  ...
  index(
    ...
    const index = indexName || Array.from(expressions.join(',').matchAll(/[a-z_]+/g)).join('_');
  ...
  setNotNull(columnName) {
    this.preSchemaTasks.push({
      type: TABLE_TASK_TYPE.addConstraint,
      perform: () =>
        this.schema
          ? this.knex.schema.raw(`alter table ??.?? alter column ?? set not null`, [
              this.schema,
              this.tableName,
              columnName,
            ])
  ...
  addHistoricalTable({
    ...
    useTextualOriginalId = false,
    uuidOriginal = false,
  } = {}) {
    this.schemaTasks.push({
      type: TABLE_TASK_TYPE.addHistoricalTable,
      perform: async () => {
        const htBuilder = new TableBuilder(
          this.knex,
          this.schema,
        ...
        htBuilder.column('id').integer().identity({ constraintName: identityConstraintName });

        if (uuidOriginal) {
          htBuilder.column('original_id').uuid().notNullable();
        } else if (useTextualOriginalId) {
          htBuilder.column('original_id').text().notNullable();
        } else {
          htBuilder.column('original_id').integer().notNullable();
        }
        ...
        htBuilder.column('user_id').text().defaultTo('system').notNullable();
        htBuilder.column('trace_id').text();

        htBuilder.timestamps();

        htBuilder.tableTasks = htBuilder.tableTasks.concat(
          this.tableTasks
            .filter(
