...
export const up = async (migrationBuilder, tenantName) => {
  ...
  const unit = await knex('companies')
    ...
    .first();
  const measurementUnitPerTenant = unit?.unit;
  const measurementUnitConstValue = `${MEASUREMENT_UNIT.us}`;

  if (measurementUnitPerTenant?.toUpperCase() === measurementUnitConstValue.toUpperCase()) {
