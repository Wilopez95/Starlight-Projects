...
describe('Approve 1 PT Delivery Order Without PO: Default Flow V1', () => {
  ...
  it(`should get newly approved delivery order and ensure that properties are correct`, async () => {
    const { statusCode, body: details } = await client
      .get(`${URL_PREFIX}/${deliveryOrderId}`)
      .query({ edit: true });
    expect(statusCode).toEqual(httpStatus.OK);
    ...
    expect(details).toHaveProperty('id');
    expect(details).toHaveProperty('status');
    expect(details.status).toEqual(ORDER_STATUS.approved);
    expect(details).toHaveProperty('isRollOff');
    expect(details.isRollOff).toEqual(false);
    expect(details).toHaveProperty('grandTotal');
    expect(details.grandTotal).toEqual(approvePtOrderInputDefault.grandTotal);
    expect(details).toHaveProperty('workOrder');
  });
  it(`should check work order of approved delivery order`, async () => {
    const { statusCode, body: details } = await client
      .get(`${URL_PREFIX}/${deliveryOrderId}`)
      .query({ edit: true });
    expect(statusCode).toEqual(httpStatus.OK);
    ...
    expect(details.workOrder.completionDate).toEqual(
      approvePtOrderInputDefault.workOrder.completionDate,
    );
  });
});
