...
const CARD_CONNECT_TXN_NOT_SETTLED = '28';
...
export const formatGqlError = error => {
  ...
  if (error instanceof apollo.ApolloError || error instanceof graphql.GraphQLError) {
    return error;
  } else if (error.handled && error.respcode === CARD_CONNECT_TXN_NOT_SETTLED) {
    // Handle some CardConnect PG errors
    return ApplicationError.preconditionFailed(error.message);
  } else {
