...
class ToastContainer extends Component {
  ...
  componentDidMount() {
    ...
    EventManager.on(SHOW, (content, options) => this.show(content, options))
      .on(CLEAR, (id) => (id === null ? this.clear() : this.removeToast(id)))
      .emit(MOUNTED, this);
  ...
  removeToast(id) {
    this.setState(
      (prevState) => ({
        toast: prevState.toast.filter((v) => v !== parseInt(id, 10)),
      }),
      this.dispatchChange
    );
  }
  ...
  show(content, options) {
    ...
    const toastOptions = {
      ...
      draggable:
        options.draggable === null ? this.props.draggable : options.draggable,
      draggablePercent:
  ...
  renderToast() {
    ...
    return Object.keys(toastToRender).map((position) => {
      ...
      const props = {
        className: cx(
          "sl-notif__toast-container",
          `sl-notif__toast-container--${position}`,
          { "sl-notif__toast-container--rtl": this.props.rtl },
          className
        ),
